...
Defects found by this run and fixed on `main`:
...
Not yet applied to the `turtlebot3-waffle` branch: the `Dockerfile` there lacks
`ros-noetic-joint-state-publisher`, `-openslam-gmapping`, `-turtlebot3-msgs`,
`-rosserial-python` and `-hls-lfcd-lds-driver` (listed in its
`install_dependencies.sh`), so `catkin_make` fails inside the image. Items 1
and 2 apply unchanged to that branch; 3 and 6 need porting.
